...
import { WorkerData, workerTable } from '@/services/worker/type'
const AvaililityTable = ({ data }: { data: WorkerData }) => {
  ...
  const dataSource = data.workers_data.map((manager: workerTable) => ({
    key: manager.id,
      availability: manager.availibility,
    email: manager.email,
    name: manager.name,
    // workers: manager.workers_count,
    wages: manager.wage,
    status: manager.status,
    branch: manager.branches.name,
  }))
...
export default AvaililityTable
